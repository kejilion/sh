...
linux_tools() {
  ...
  while true; do
      ...
      echo -e "▶ 常用工具"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}1.   ${gl_bai}curl 下载工具 ${gl_huang}★${gl_bai}                   ${gl_lan}2.   ${gl_bai}wget 下载工具 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lan}3.   ${gl_bai}sudo 超级管理权限工具             ${gl_lan}4.   ${gl_bai}socat 通信连接工具"
      echo -e "${gl_lan}5.   ${gl_bai}htop 系统监控工具                 ${gl_lan}6.   ${gl_bai}iftop 网络流量监控工具"
      echo -e "${gl_lan}7.   ${gl_bai}unzip ZIP压缩解压工具             ${gl_lan}8.   ${gl_bai}tar GZ压缩解压工具"
      echo -e "${gl_lan}9.   ${gl_bai}tmux 多路后台运行工具             ${gl_lan}10.  ${gl_bai}ffmpeg 视频编码直播推流工具"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}11.  ${gl_bai}btop 现代化监控工具 ${gl_huang}★${gl_bai}             ${gl_lan}12.  ${gl_bai}ranger 文件管理工具"
      echo -e "${gl_lan}13.  ${gl_bai}gdu 磁盘占用查看工具              ${gl_lan}14.  ${gl_bai}fzf 全局搜索工具"
      echo -e "${gl_lan}15.  ${gl_bai}vim 文本编辑器                    ${gl_lan}16.  ${gl_bai}nano 文本编辑器 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lan}13.  ${gl_bai}git 版本控制系统"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}21.  ${gl_bai}黑客帝国屏保                      ${gl_lan}22.  ${gl_bai}跑火车屏保"
      echo -e "${gl_lan}26.  ${gl_bai}俄罗斯方块小游戏                  ${gl_lan}27.  ${gl_bai}贪吃蛇小游戏"
      echo -e "${gl_lan}28.  ${gl_bai}太空入侵者小游戏"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}31.  ${gl_bai}全部安装                          ${gl_lan}32.  ${gl_bai}全部安装（不含屏保和游戏）${gl_huang}★${gl_bai}"
      echo -e "${gl_lan}33.  ${gl_bai}全部卸载"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}41.  ${gl_bai}安装指定工具                      ${gl_lan}42.  ${gl_bai}卸载指定工具"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}0.   ${gl_bai}返回主菜单"
...
linux_test() {
    ...
    while true; do
      ...
      echo -e "▶ 测试脚本合集"
      echo -e ""
      echo -e "${gl_lv}IP及解锁状态检测"
      echo -e "${gl_lv}1.   ${gl_bai}ChatGPT解锁状态检测"
      echo -e "${gl_lv}2.   ${gl_bai}Region流媒体解锁测试"
      echo -e "${gl_lv}3.   ${gl_bai}yeahwu流媒体解锁检测"
      echo -e "${gl_lv}4.   ${gl_bai}xykt_IP质量体检脚本 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lv}"
      echo -e "${gl_lv}网络线路测速"
      echo -e "${gl_lv}11.  ${gl_bai}besttrace三网回程延迟路由测试"
      echo -e "${gl_lv}12.  ${gl_bai}mtr_trace三网回程线路测试"
      echo -e "${gl_lv}13.  ${gl_bai}Superspeed三网测速"
      echo -e "${gl_lv}14.  ${gl_bai}nxtrace快速回程测试脚本"
      echo -e "${gl_lv}15.  ${gl_bai}nxtrace指定IP回程测试脚本"
      echo -e "${gl_lv}16.  ${gl_bai}ludashi2020三网线路测试"
      echo -e "${gl_lv}17.  ${gl_bai}i-abc多功能测速脚本"
      echo -e "${gl_lv}"
      echo -e "${gl_lv}硬件性能测试"
      echo -e "${gl_lv}21.  ${gl_bai}yabs性能测试"
      echo -e "${gl_lv}22.  ${gl_bai}icu/gb5 CPU性能测试脚本"
      echo -e "${gl_lv}"
      echo -e "${gl_lv}综合性测试"
      echo -e "${gl_lv}31.  ${gl_bai}bench性能测试"
      echo -e "${gl_lv}32.  ${gl_bai}spiritysdx融合怪测评 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lv}"
      echo -e "${gl_lv}------------------------"
...
linux_Oracle() {
     ...
     while true; do
      ...
      echo -e "▶ 甲骨文云脚本合集"
      echo -e "${gl_hong}------------------------"
      echo -e "${gl_hong}1.   ${gl_bai}安装闲置机器活跃脚本"
      echo -e "${gl_hong}2.   ${gl_bai}卸载闲置机器活跃脚本"
      echo -e "${gl_hong}------------------------"
      echo -e "${gl_hong}3.   ${gl_bai}DD重装系统脚本"
      echo -e "${gl_hong}4.   ${gl_bai}R探长开机脚本"
      echo -e "${gl_hong}------------------------"
      echo -e "${gl_hong}5.   ${gl_bai}开启ROOT密码登录模式"
      echo -e "${gl_hong}------------------------"
...
linux_work() {
    ...
    while true; do
      ...
      echo -e "${gl_zi}------------------------"
      echo -e "${gl_zi}1.   ${gl_bai}1号工作区"
      echo -e "${gl_zi}2.   ${gl_bai}2号工作区"
      echo -e "${gl_zi}3.   ${gl_bai}3号工作区"
      echo -e "${gl_zi}4.   ${gl_bai}4号工作区"
      echo -e "${gl_zi}5.   ${gl_bai}5号工作区"
      echo -e "${gl_zi}6.   ${gl_bai}6号工作区"
      echo -e "${gl_zi}7.   ${gl_bai}7号工作区"
      echo -e "${gl_zi}8.   ${gl_bai}8号工作区"
      echo -e "${gl_zi}9.   ${gl_bai}9号工作区"
      echo -e "${gl_zi}10.  ${gl_bai}10号工作区"
      echo -e "${gl_zi}------------------------"
      echo -e "${gl_zi}99.  ${gl_bai}工作区管理 ${gl_huang}★${gl_bai}"
      echo -e "${gl_zi}------------------------"
      echo -e "${gl_zi}0.   ${gl_bai}返回主菜单"
      echo -e "${gl_zi}------------------------${gl_bai}"
      read -p "请输入你的选择: " sub_choice
...
linux_Settings() {
    ...
    while true; do
      ...
      echo -e "▶ 系统工具"
      echo -e "${gl_lv}------------------------"
      echo -e "${gl_lv}1.   ${gl_bai}设置脚本启动快捷键                 ${gl_lv}2.   ${gl_bai}修改登录密码"
      echo -e "${gl_lv}3.   ${gl_bai}ROOT密码登录模式                   ${gl_lv}4.   ${gl_bai}安装Python指定版本"
      echo -e "${gl_lv}5.   ${gl_bai}开放所有端口                       ${gl_lv}6.   ${gl_bai}修改SSH连接端口"
      echo -e "${gl_lv}7.   ${gl_bai}优化DNS地址                        ${gl_lv}8.   ${gl_bai}一键重装系统 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lv}9.   ${gl_bai}禁用ROOT账户创建新账户             ${gl_lv}10.  ${gl_bai}切换优先ipv4/ipv6"
      echo -e "${gl_lv}------------------------"
      echo -e "${gl_lv}11.  ${gl_bai}查看端口占用状态                   ${gl_lv}12.  ${gl_bai}修改虚拟内存大小"
      echo -e "${gl_lv}13.  ${gl_bai}用户管理                           ${gl_lv}14.  ${gl_bai}用户/密码生成器"
      echo -e "${gl_lv}15.  ${gl_bai}系统时区调整                       ${gl_lv}16.  ${gl_bai}设置BBR3加速"
      echo -e "${gl_lv}17.  ${gl_bai}防火墙高级管理器                   ${gl_lv}18.  ${gl_bai}修改主机名"
      echo -e "${gl_lv}19.  ${gl_bai}切换系统更新源                     ${gl_lv}20.  ${gl_bai}定时任务管理"
      echo -e "${gl_lv}------------------------"
      echo -e "${gl_lv}21.  ${gl_bai}本机host解析                       ${gl_lv}22.  ${gl_bai}fail2banSSH防御程序"
      echo -e "${gl_lv}23.  ${gl_bai}限流自动关机                       ${gl_lv}24.  ${gl_bai}ROOT私钥登录模式"
      echo -e "${gl_lv}25.  ${gl_bai}TG-bot系统监控预警                 ${gl_lv}26.  ${gl_bai}修复OpenSSH高危漏洞（岫源）"
      echo -e "${gl_lv}27.  ${gl_bai}红帽系Linux内核升级                ${gl_lv}28.  ${gl_bai}Linux系统内核参数优化 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lv}29.  ${gl_bai}病毒扫描工具 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lv}------------------------"
      echo -e "${gl_lv}31.  ${gl_bai}留言板                             ${gl_lv}66.  ${gl_bai}一条龙系统调优 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lv}------------------------"
      echo -e "${gl_lv}99.  ${gl_bai}重启服务器                         ${gl_lv}100. ${gl_bai}隐私与安全"
      echo -e "${gl_lv}------------------------"
      echo -e "${gl_lv}101. ${gl_bai}卸载科技lion脚本"
      echo -e "${gl_lv}------------------------"
      echo -e "${gl_lv}0.   ${gl_bai}返回主菜单"
      echo -e "${gl_lv}------------------------${gl_bai}"
      read -p "请输入你的选择: " sub_choice
...
linux_cluster() {
    ...
    while true; do
      ...
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}1.   ${gl_bai}安装集群环境"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}2.   ${gl_bai}集群控制中心 ${gl_huang}★${gl_bai}"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}7.   ${gl_bai}备份集群环境"
      echo -e "${gl_lan}8.   ${gl_bai}还原集群环境"
      echo -e "${gl_lan}9.   ${gl_bai}卸载集群环境"
      echo -e "${gl_lan}------------------------"
      echo -e "${gl_lan}0.   ${gl_bai}返回主菜单"
      echo -e "${gl_lan}------------------------${gl_bai}"
      read -p "请输入你的选择: " sub_choice
