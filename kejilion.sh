...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}115. ${color115}Hermes机器人管理工具${gl_huang}★${gl_bai}             ${gl_kjlan}116. ${color116}DeepSeek Hermes基础轻量版${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}-------------------------"
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  116|deepseek-hermes|DeepSeek-Hermes|ds-hermes)
		  bash <(curl -fsSL ${gh_proxy}raw.githubusercontent.com/kejilion/sh/main/deepseek_hermes_manager.sh)
		  ;;
