...
kpanel_node_join() {
	...
	if [ "$existing_config" = "true" ]; then
		...
		if [ -n "$token_fingerprint" ] && [ "$saved_fingerprint" = "$token_fingerprint" ]; then
			...
			echo "The same node authorization was detected; continue enabling native services."
		else
			echo "A new node authorization was detected; run k kpanel node uninstall before connecting the new node."
	...
	if [ "$resume_enrollment" != "true" ]; then
		...
		if [ -n "$token_fingerprint" ] && ! kpanel_node_save_enrollment_fingerprint "$token_fingerprint"; then
			echo "The node authorization was saved, but the local resume marker could not be written; retry with a new access command." >&2
			return 1
		fi
	fi
...
kpanel_node_update() {
	...
	if ! kpanel_node_write_uninstall_helper || ! kpanel_node_write_units || ! kpanel_node_activate; then
		echo "KPanel lightweight node update completed, but enabling the remote uninstall trigger failed; repair systemd and run the update command again." >&2
		return 1
	fi
