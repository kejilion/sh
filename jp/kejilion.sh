		...
		check_cf_mode
			  ...
			  case $sub_choice in
				  ...
				  22)
					  ...
					  echo -e "${gl_huang}Web サイトは 5 分ごとに自動的に検出します。高負荷を検知すると自動的にシールドが開きます。負荷が低い場合は、シールドが 5 秒間自動的に閉じます。${gl_bai}"
					  echo "--------------"
					  echo "CF パラメータを取得します。"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}9.   ${gl_bai}ROOTアカウントを無効にして新しいアカウントを作成する${gl_kjlan}10.  ${gl_bai}優先順位をipv4/ipv6に切り替える"
	  echo -e "${gl_kjlan}------------------------"
