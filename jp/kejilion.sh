		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				if [[ "$running_count" -gt 0 ]]; then
					echo -e "${YELLOW}Compose 项目 [$project_name] コンテナはすでに実行されているため、復元をスキップします...${NC}"
					continue
				fi
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}テクノロジ ライオン スクリプトをインストールする${gl_kjlan}12. ${gl_bai}更新系统              ${gl_kjlan}13. ${gl_bai}システムをクリーンアップする"
