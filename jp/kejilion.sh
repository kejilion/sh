...
kpanel_run_docker_app_install() {
	...
	kpanel_app_progress 30 "kejilion.sh アプリケーションインストール関数の実行"
		...
		case $choice in
			1)
				...
				if docker_app_install; then
					...
					echo -e "${gl_hong}インストールに失敗しました:${gl_bai}未登记应用状态，请根据上方错误修复后重试。"
					action_status=1
				fi
				;;
			...
			2)
				if docker_app_update; then
					...
					echo -e "${gl_hong}更新に失敗しました:${gl_bai}已保留原应用登记状态。"
					action_status=1
				fi
				;;
...
kpanel_node_join() {
	...
	if [ "$existing_config" = "true" ]; then
		...
		if [ -n "$token_fingerprint" ] && [ "$saved_fingerprint" = "$token_fingerprint" ]; then
			...
			echo "同じノード認証が検出されました。ネイティブ サービスの有効化を続行します。"
		else
			echo "新しいノード認証が検出されました。新しいノードを接続する前に k kpanel node uninstall を実行します。"
	...
	if [ "$resume_enrollment" != "true" ]; then
		...
		if [ -n "$token_fingerprint" ] && ! kpanel_node_save_enrollment_fingerprint "$token_fingerprint"; then
			echo "ノード認証は保存されましたが、ローカルの再開マーカーを書き込めませんでした。新しいアクセス コマンドで再試行してください。" >&2
			return 1
		fi
	fi
...
refresh_apps_catalog() {
	...
	if [ -e "$apps_dir" ] && [ ! -d "$apps_dir/.git" ]; then
		echo -e "${gl_hong}错误: ${gl_bai}${apps_dir}すでに存在しますが、App Market Git リポジトリではないため、上書きは拒否されます。"
		return 1
	fi
