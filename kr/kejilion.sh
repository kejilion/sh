...
sync-openclaw-provider-interactive() {
	...
	read -erp "동기화할 API 이름(provider), Enter로 전체 동기화: " provider_name
	if [ -z "$provider_name" ]; then
		if sync_openclaw_api_models; then
			start_gateway
		else
			echo "❌ API 모델 동기 실패, 게이트웨이 재시작 중단. provider /models 확인 후 재시도하세요."
			break_end
			return 1
		fi
		return 0
...
while true; do
	  ...
	  case $sub_choice in
		  ...
		  4)
			  clear
			  send_stats "备份集群"
			  echo -e "请将 ${gl_huang}${HOME}/cluster/servers.py${gl_bai} 文件下载，完成备份！"
			  break_end
			  ;;
		  ...
		  5)
			  ...
			  echo -e "请上传您的 ${gl_huang}servers.py${gl_bai} 文件到 ${gl_huang}${HOME}/cluster/${gl_bai} 完成还原！"
			  break_end
			  ;;
